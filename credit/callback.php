...
if (isset($_SESSION['accessId'])) {
    ...
    // ========== ペア入会情報の取得 START ==========
    $pairMember = $_SESSION['data'][$accessId]['pairMember'] ?? null;
    $pairEntry = $_SESSION['data'][$accessId]['pairEntry'] ?? null;
    // ========== ペア入会情報の取得 END ==========
} else {
    header("Location:"."https://applegym.jp");
}
...
$chargeNextMonth = $selectedDate->copy()->addMonthNoOverflow(1)->day(1)->format('Ymd');
$startChargeDate = $selectedDate->copy()->addMonthNoOverflow(1)->day(10)->format('Y-m-d');
...
if($error == null) {
    ...
    $mailTo = $email;
    $mailAdminTo = "info@applegym-test.net";
    $bcc = "n.senzaki@rexiv.co.jp, n.takase@rexiv.co.jp, apple.t.kawamura@gmail.com";

    $totalAmountPrice = $price; // 変数名の修正
    ...
    // ========== ペア会員のカード登録 START ==========
    if($pairEntry == '1' && $pairMember && $error == null) {
        $pairStoreCardParams = [
            "merchant" => $merchant,
            "creditStoringInformation" => [
                "referrer" => [
        ...
        curl_setopt($ch, CURLOPT_URL, $oauthApiUrl.'credit/storeCard');
        curl_setopt($ch, CURLOPT_POST, true);
        curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($pairStoreCardParams));
        curl_setopt($ch, CURLOPT_HTTPHEADER, [
            'Content-Type: application/json',
            'Authorization: ' . 'Bearer '. $accessToken,
            'X-MP-Version: 2024-12'
        ...
        $pairStoreResponse = curl_exec($ch);
        $pairStoreInfo = curl_getinfo($ch);
        
        if ($pairStoreInfo['http_code'] != 201) {
            error_log("[AppleGYM] ペア会員のカード登録失敗: " . $pairStoreResponse);
        }
        curl_close($ch);
    }
    // ========== ペア会員のカード登録 END ==========
    ...
    if($error == null) {
        ...
        // ========== ペア会員の定期払い設定 START ==========
        if($pairEntry == '1' && $pairMember && !array_key_exists('ErrCode', $data) && $error == null) {
            $pairRecurringID = "o-" . substr(str_shuffle($recurringStr), 0, 8);
            
            $pairRegisterParams = [
                'ShopID'           => $shopId,
                'ShopPass'         => $shopPass,
                'RecurringID'      => $pairRecurringID,
                'Amount'           => $subscriptionPrice,
                'ChargeDay'        => 10,
                'ChargeMonth'      => "01|02|03|04|05|06|07|08|09|10|11|12",
                'ChargeStartDate'  => $chargeNextMonth,
                ...
                'SitePass'         => $sitePass,
                'MemberID'         => $pairMember,
                'ClientField1'     => $buyItemsName . "_ペア",
            ];
            
            $encodedPairParam = [];
            foreach ($pairRegisterParams as $key => $value) {
                $encodedPairParam[] = urlencode($key) . '=' . urlencode(mb_convert_encoding($value, 'SJIS-win', 'UTF-8'));
            }
            $pairRequestBody = implode('&', $encodedPairParam);
            
            $curl = curl_init();
            curl_setopt($curl, CURLOPT_POST, true);
            curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($curl, CURLOPT_URL, $apiUrl.'payment/RegisterRecurringCredit.idPass');
            curl_setopt($curl, CURLOPT_POSTFIELDS, $pairRequestBody);
            curl_setopt($curl, CURLOPT_HTTPHEADER, [
                'Content-Type: application/x-www-form-urlencoded;charset=windows-31j',
            ]);
            
            $pairRegisterResponse = curl_exec($curl);
            $pairCurlinfo = curl_getinfo($curl);
            curl_close($curl);
            
            if ($pairCurlinfo['http_code'] != 200) {
                error_log("[AppleGYM] ペア会員の定期払い設定失敗: " . $pairRegisterResponse);
            }
        }
        // ========== ペア会員の定期払い設定 END ==========
        ...
        if (array_key_exists('ErrCode', $data) || $error != null) {
            ...
            // ========== ペア用の自動売上ID情報 START ==========
            $recurringInfo = "【 自動売上ID 】{$recurringID}";
            if($pairRecurringID) {
                $recurringInfo .= "\n【 ペア自動売上ID 】{$pairRecurringID}";
            }
            // ========== ペア用の自動売上ID情報 END ==========
